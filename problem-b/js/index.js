...
function createTaskItemElement(taskitem) {
    ...
    t1.textContent = taskitem.description;
    if (taskitem.complete == true) {
        t1.className = 'font-strike';
    }
...
function renderTaskList() {
  ...
  for (let i = 0; i < state.taskList.length; i++) {
      olElem.append(createTaskItemElement(state.taskList[i]));
  }
  renderInput()
}
...
function addNewTask() {
  state.taskList.push({id: state.taskList.length + 1,
...
let listeneritem = document.querySelector('#add-task');
listeneritem.addEventListener('click', addNewTask);
